...
Separately, every endpoint's representative requests and actual source-engine
200/400 results are checked against its generated OpenAPI operation; the
sanitized internal-error object is checked against the 500 schema. Negative
...
`artifact-manifest.json` hashes all 43 public generated and machine-contract artifacts: the catalog, portfolio, contract registry, API and policy contracts, the policy lifecycle and runtime compatibility plans, fourteen JSON Schemas, plus each workflow, companion README, and three fixtures. Validation recomputes every byte count and SHA-256 digest so generated files cannot drift independently of their recorded release identity.
...
The versioned `runtime-compatibility.json` contract is the source of truth for the pinned n8n/Node matrix, representative workflow, and required probes. The weekly compatibility workflow consumes that file, imports the complete catalog into each isolated n8n version, publishes one credential-free representative workflow, and sends low-risk, high-risk, invalid-fixture, explicit-null, and non-JSON media-type requests through the real production webhook path. It then imports a test-only copy whose evaluator deliberately throws and proves the error output returns the sanitized 500 contract. This catches parser and runtime behavior that JSON import validation cannot. n8n currently normalizes an HTTP JSON `null` payload to an empty object before workflow execution, so the live null probe requires a contract-valid 400 but does not require the engine's direct root-type violation.
