...
`artifact-manifest.json` hashes all 88 public generated and machine-contract artifacts: the catalog, API and policy contracts, the policy lifecycle and runtime compatibility plans, six JSON Schemas, plus each workflow, companion README, and three fixtures. Validation recomputes every byte count and SHA-256 digest so generated files cannot drift independently of their recorded release identity.
