...
- **Generated-file substitution:** `artifact-manifest.json` records the byte size and SHA-256 identity of every public generated and contract artifact and is recomputed during validation.
...
- **Credential leakage outside workflow JSON:** `npm run scan:sensitive` scans the complete repository text tree with redacted findings for private keys, provider tokens, JWTs, and high-entropy credential assignments; `npm run check` enforces it in CI. Reviewed false positives may use `secret-scan: allow` on the same line, but never use an exception for a real secret.
