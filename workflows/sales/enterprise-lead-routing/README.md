...
The high-risk example returns `assign_enterprise_owner` in the `high` band with score 70. Invalid requests return HTTP 400 with `error: "validation_error"`, field-level violations, and the complete request schema so callers can self-correct.
