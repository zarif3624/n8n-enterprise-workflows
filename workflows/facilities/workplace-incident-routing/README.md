# Route workplace incidents safely

Prioritizes workplace incidents using injury, immediate danger, access control, operational disruption, and people-impact signals.

## Business problem

Facilities incidents arrive through inconsistent channels, delaying the right safety, security, or operations response.

## Business outcome

A defensible urgency tier with clear escalation reasons and a named human response path.

- **Primary owner:** Workplace Operations
- **Primary metric:** Minutes from report to accountable responder
- **Policy version:** `1.0.1`
- **ROI starting point:** `workplace incidents x minutes faster routing x disruption cost per minute`

## Five-minute adoption

1. Import `workflow.json` into an n8n development project.
2. Review the policy rules, hard risk gates, and thresholds with the named business owner.
3. Keep the workflow inactive while testing. The imported webhook is intentionally unauthenticated for local evaluation.
4. Send `examples/low-risk.json`, `examples/high-risk.json`, and `examples/invalid.json` to the test webhook URL.
5. Configure Header Auth, Basic Auth, or another approved built-in webhook credential before activation.
6. Connect approved downstream systems only after the decision contract is verified.

```bash
curl --fail-with-body --request POST "$N8N_TEST_WEBHOOK_URL" \
  --header "Content-Type: application/json" \
  --header "X-Request-Id: local-test-001" \
  --data @examples/low-risk.json
```

## Input contract

The request body must be a JSON object. Unknown fields are accepted for caller compatibility but ignored by the policy and never echoed in the response.

| Field | Required | Contract |
| --- | --- | --- |
| `incidentId` | Yes | string |
| `site` | Yes | string |
| `category` | Yes | string |
| `description` | Yes | string |
| `injuryReported` | No | boolean |
| `immediateDanger` | No | boolean |
| `accessControlImpact` | No | boolean |
| `operationsDisrupted` | No | boolean |
| `peopleAffected` | No | number, min 0 |

## Policy rules

| Rule ID | Field | Match | Points | Minimum band | Reason |
| --- | --- | --- | ---: | --- | --- |
| `injuryReported_truthy_1` | `injuryReported` | truthy | 55 | high | An injury has been reported |
| `immediateDanger_truthy_2` | `immediateDanger` | truthy | 70 | high | People may face immediate danger |
| `accessControlImpact_truthy_3` | `accessControlImpact` | truthy | 30 | — | Physical access controls are affected |
| `operationsDisrupted_truthy_4` | `operationsDisrupted` | truthy | 25 | — | Workplace operations are disrupted |
| `peopleAffected_gte_5` | `peopleAffected` | gte 25 | 25 | — | At least 25 people are affected |

Scores below 30 use `route_standard_facilities_queue`, scores from 30-69 use `dispatch_urgent_workplace_response`, and scores of 70+ use `activate_safety_and_security_response`. A minimum band is a hard safety floor: negative rules cannot cancel it.

## Response contract

Successful requests return HTTP 200 with a request ID, policy version, decision, band, score, matched rules, recommended actions, and evaluation timestamp.

```json
{
  "ok": true,
  "httpStatus": 200,
  "requestId": "example-request-001",
  "workflow": "workplace-incident-routing",
  "policyVersion": "1.0.1",
  "decision": "route_standard_facilities_queue",
  "priorityBand": "low",
  "score": 0,
  "matchedRules": [],
  "recommendedActions": [
    "Create the facilities incident",
    "Assign an on-site response owner",
    "Require human confirmation before closing a safety event"
  ],
  "evaluatedAt": "2026-08-07T03:00:00.000Z"
}
```

The high-risk example returns `activate_safety_and_security_response` in the `high` band with score 100. Invalid requests return HTTP 400 with `error: "validation_error"`, field-level violations, and the complete request schema so callers can self-correct. Unexpected evaluator failures follow the wired error output and return a sanitized, retryable HTTP 500 with `error: "internal_error"`; stack traces and caller data are never returned.

## Recommended production extensions

- Create the facilities incident.
- Assign an on-site response owner.
- Require human confirmation before closing a safety event.

Typical adapters: facilities management, physical security, Slack, Microsoft Teams.

## Security and operations

- Keep consequential actions behind explicit human approval. This template recommends; it does not make irreversible changes.
- Replace unauthenticated local testing with built-in webhook authentication before activation.
- Store secrets only in n8n credentials; never place tokens in expressions, fields, or exported JSON.
- Add retries only to safe, idempotent external calls and wire every fallible node to a structured 5xx response.
- Set a private workflow-level error workflow and review execution-data retention before processing sensitive data.
- Treat `requestId` as a correlation value, not proof of identity, and avoid returning source records in the response.

## ROI worksheet

Start with `workplace incidents x minutes faster routing x disruption cost per minute`. Record baseline volume, handling time, false-positive rate, and loaded cost before rollout; compare them with observed values after 30 days. Report capacity, risk reduction, and revenue impact separately.

## Search terms

n8n facilities automation, enterprise n8n workflow, workplace incident routing, workflow automation template.
